...
def open_new_window():
    ...
    def button_click():
        nonlocal start_time
        root.after(1000, lambda: button.config(bg='green'))
        start_time = time.time()

    def button_release(event):
        nonlocal start_time
        elapsed_time = round((time.time() - start_time) - 1, 3)
    ...
    start_time = None
    button = tk.Button(new_window, text="Training reaction", command=button_click, bg='blue', height=10, width=100)
...
table_label = tk.Label(root, text="Топ пользователей:")
